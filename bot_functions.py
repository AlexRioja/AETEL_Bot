...
def checkNepePoints(bot, update, job_queue, chat_data):
    ...
    for k,v in config.users.items():
        if k == username:
            ...
            job = job_queue.run_once(deleteMessage, 40, context=update.message)
            job = job_queue.run_once(deleteMessage, 42, context=msg_sent)
            chat_data['job'] = job
...
def giveNepePoints(bot, update, job_queue, chat_data):
    ...
    if username not in config.allowed_usernames:
        ...
        job = job_queue.run_once(deleteMessage, 40, context=msg_sent)
    else:
        ...
        for k,v in config.users.items():
            if k == splitted[1][1:]:
                ...
                job = job_queue.run_once(deleteMessage, 40, context=msg_sent)
        #print("MSG-SENT->"+str(msg_sent))
        job = job_queue.run_once(deleteMessage, 42, context=update.message)
